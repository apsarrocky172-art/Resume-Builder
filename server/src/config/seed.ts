...
export const seedDatabase = async () => {
  try {
    ...
    const { error: delQErr } = await supabase.from('questions').delete().neq('question_text', '');
    if (delQErr) console.warn('[Seeder] Warn/Error clearing questions:', delQErr.message);

    const { error: delJErr } = await supabase.from('jobs').delete().neq('title', '');
    if (delJErr) console.warn('[Seeder] Warn/Error clearing jobs:', delJErr.message);

    console.log('[Seeder] Seeding Aptitude & Coding Questions to Supabase...');
    const questionsToInsert = ([...aptitudeQuestions, ...codingQuestions] as any[]).map(q => ({
      type: q.type,
      category: q.category,
    ...
    const { error: insQErr } = await supabase.from('questions').insert(questionsToInsert);
    ...
    const { error: insJErr } = await supabase.from('jobs').insert(jobsToInsert);
    if (insJErr) throw insJErr;
