...
import { generateAptitudeQuestionsAI, generateCodingChallengesAI, chatAssistantAI, generateTechnicalQuizAI } from '../config/ai';
import vm from 'vm';
...
export const getAptitudeQuestions = async (req: Request, res: Response) => {
  try {
    ...
    const cat = category as string || 'mixed';
    ...
    if (!dbError && dbQuestions && dbQuestions.length > 0) {
      if (dbQuestions.length >= size) {
        ...
        return res.status(200).json(shuffled.map((q: any) => ({
          _id: q.id,
          type: q.type,
          category: q.category,
      ...
      try {
        ...
        const questions = await generateAptitudeQuestionsAI(cat, size - dbQuestions.length);
    ...
    const questions = await generateAptitudeQuestionsAI(cat, size);
    ...
    res.status(200).json(savedQuestions.map((q: any) => ({
      _id: q.id,
      type: q.type,
      category: q.category,
      difficulty: q.difficulty,
...
export const getCodingChallenges = async (req: Request, res: Response) => {
  try {
    ...
    const diff = difficulty as string || 'medium';
    ...
    if (!dbError && dbChallenges && dbChallenges.length > 0) {
      ...
      challenges = dbChallenges.map((c: any) => ({
        _id: c.id,
        type: c.type,
        category: c.category,
  ...
  try {
    ...
    if (language === 'javascript') {
      try {
        ...
        for (const tc of testCases) {
          ...
          const runScript = new vm.Script(`${functionName}(${tc.input})`);
    ...
    if (isUuid(questionId)) {
      try {
        ...
        if (sError) console.error('[Trainer] Error saving submission:', sError.message);
      } catch (err: any) {
...
export const botChat = async (req: Request, res: Response) => {
  try {
    const { message, history } = req.body;
    if (!message) {
      return res.status(400).json({ message: 'Message is required' });
    }
    const reply = await chatAssistantAI(message, history || []);
...
function calculateCodingStreak(submissions: any[]): number {
...
function createTestFromGroup(group: any[]): any {
...
function groupAptitudeSubmissionsIntoTests(submissions: any[]): any[] {
...
export const getDashboardStatsForUser = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) return res.status(401).json({ message: 'Unauthorized' });
    ...
    const [resumeResult, interviewsResult, submissionsResult] = await Promise.all([
      supabase.from('resumes').select('resume_score').eq('user_id', userId).maybeSingle(),
      supabase.from('interviews').select('scores').eq('user_id', userId).eq('status', 'completed'),
      supabase.from('submissions').select('*, questions(type, category)').eq('user_id', userId)
    ]);
    ...
    const codingSubmissions = (submissions || []).filter((s: any) => s.language !== 'aptitude');
    const aptitudeSubmissions = (submissions || []).filter((s: any) => s.language === 'aptitude');
    ...
    const testHistory = [];
    const sortedTests = [...tests].sort((a: any, b: any) => new Date(a.date).getTime() - new Date(b.date).getTime());
    ...
    const quantScores = aptitudeSubmissions.filter((s: any) => {
    ...
    const logicalScores = aptitudeSubmissions.filter((s: any) => {
      const cat = (s.questions && (Array.isArray(s.questions) ? s.questions[0]?.category : s.questions.category) || '').toLowerCase();
      return cat.includes('logical') || cat.includes('sufficiency');
    });
    const verbalScores = aptitudeSubmissions.filter((s: any) => {
      const cat = (s.questions && (Array.isArray(s.questions) ? s.questions[0]?.category : s.questions.category) || '').toLowerCase();
    ...
    const acceptedCoding = codingSubmissions.filter((s: any) => s.status === 'Accepted').length;
    const codingAccuracy = codingSubmissions.length > 0 ? Math.round((acceptedCoding / codingSubmissions.length) * 100) : 0;

    const communicationScores = (completedInterviews || []).map((i: any) => i.scores?.communication || 0);
...
export const getTechnicalQuizQuestions = async (req: Request, res: Response) => {
  try {
    const { role, level, exclude, limit } = req.query;
    const targetRole = role as string || 'Software Engineer';
    const targetLevel = level as string || 'beginner';
    const size = limit ? parseInt(limit as string, 10) : 20;
    ...
    if (excludeList.length > 0) {
      filteredQuestions = filteredQuestions.filter((q: any) => !excludeList.includes(q.question_text));
    }
    ...
    const generated = await generateTechnicalQuizAI(targetRole, targetLevel, excludeList, size);
...
export const submitTechnicalQuiz = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) return res.status(401).json({ message: 'Unauthorized' });
