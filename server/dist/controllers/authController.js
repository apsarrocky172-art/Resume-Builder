...
exports.updateProfile = exports.getMe = exports.forgotPassword = exports.login = exports.register = void 0;
...
const login = async (req, res) => {
    try {
        ...
        let authResult = await db_1.supabase.auth.signInWithPassword({
            email,
            password
        });
        if (authResult.error) {
            if (email === 'google.student@university.edu' || email.endsWith('@gmail.com') || email.endsWith('.edu') || email.endsWith('@apsardev.com')) {
                ...
                const formattedName = namePart.split(/[^a-zA-Z]/).map((word) => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
                console.log(`[Auth] Mock Google student (${email}) not found. Auto-registering as ${formattedName}...`);
                const signUpResult = await db_1.supabase.auth.signUp({
                    email,
                ...
                if (!signUpResult.error) {
                    authResult = await db_1.supabase.auth.signInWithPassword({
                        email,
                        password: 'googlesecret_123'
                    });
                }
        ...
        let { data: userProfile, error: profileError } = await db_1.supabase
            .from('users')
            .select('*')
            .eq('id', data.user.id)
            .single();
        if (profileError || !userProfile) {
            ...
            const { data: syncedProfile, error: syncError } = await db_1.supabase
                .from('users')
                .insert([{
                    id: data.user.id,
        ...
        if (email === 'admin@placement.edu' && userProfile && userProfile.role !== 'admin') {
            ...
            const { data: updatedProfile, error: updateError } = await db_1.supabase
                .from('users')
                .update({ role: 'admin' })
                .eq('id', data.user.id)
                .select()
...
const getMe = async (req, res) => {
    try {
        ...
        let { data: user, error } = await db_1.supabase
            .from('users')
            .select('*')
            .eq('id', req.user.id)
            .single();
...
const updateProfile = async (req, res) => {
    try {
        ...
        await db_1.supabase.auth.updateUser({
            data: { name }
        });
